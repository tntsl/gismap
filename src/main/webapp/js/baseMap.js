...
function queryWorkArea(geometries) {
	var query = new esri.tasks.Query();
	var queryTask = new esri.tasks.QueryTask(mapservice_l + "/5");
	query.geometry = geometries;
	query.where = "1=1";
	query.returnGeometry = true;
	query.outFields = ["*"];
...
function queryRescueTeam(geometries) {
	var query = new esri.tasks.Query();
	var queryTask = new esri.tasks.QueryTask(mapservice_l + "/8");
	query.geometry = geometries;
	query.where = "1=1";
	query.returnGeometry = true;
	query.outFields = ["*"];
...
function FlyToXYEvent(X, Y, Zoom) {
	...
	var symbol = new esri.symbol.PictureMarkerSymbol(ctxStatic + "/icon/fly.gif", 16, 13);
...
function showBookMarks() {
	...
	$.post(ctx + "/arcmap/bookMarks.json", function(data) {
