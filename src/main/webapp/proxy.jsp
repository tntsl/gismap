...
<%!public static final class DataValidUtil {
	...
	String PROXY_REFERER = "http://localhost:8080/ictmap/proxy.jsp";
	...
	private HttpURLConnection forwardToServer(HttpServletRequest request, String uri, byte[] postBody) throws IOException {
		...
		return postBody.length > 0 ? doHTTPRequest(uri, postBody, "POST", mapHeaderInfo) : doHTTPRequest(uri, request.getMethod());
	}
	...
	private String getNewTokenIfCredentialsAreSpecified(ServerUrl su, String url) throws IOException {
		...
		if (isUserLogin || isAppLogin) {
			...
			if (isAppLogin) {
				...
				String uri = su.getOAuth2Endpoint() + "token?client_id=" + URLEncoder.encode(su.getClientId(), "UTF-8") + "&client_secret=" + URLEncoder.encode(su.getClientSecret(), "UTF-8")
				...
				if (!infoUrl.isEmpty()) {
					...
					if (tokenServiceUri != null && !tokenServiceUri.isEmpty()) {
						...
						String uri = tokenServiceUri + "?f=json&request=getToken&referer=" + URLEncoder.encode(PROXY_REFERER, "UTF-8") + "&expiration=60&username="
								+ URLEncoder.encode(su.getUsername(), "UTF-8") + "&password=" + URLEncoder.encode(su.getPassword(), "UTF-8");
						String tokenResponse = webResponseToString(doHTTPRequest(uri, "POST"));
						token = extractToken(tokenResponse, "token");
					}
				}
	...
	private boolean checkReferer(String[] allowedReferers, String referer) throws MalformedURLException {
		if (allowedReferers != null && allowedReferers.length > 0) {
			...
			for (String allowedReferer : allowedReferers) {
				...
				if (protocolMatch(allowedRefererURL.getProtocol(), refererURL.getProtocol()) && domainMatched(allowedRefererURL.getHost(), refererURL.getHost())
	...
	private String exchangePortalTokenForServerToken(String portalToken, ServerUrl su) throws IOException {
		...
		String uri = su.getOAuth2Endpoint().substring(0, su.getOAuth2Endpoint().toLowerCase().indexOf("/oauth2/")) + "/generateToken?token=" + URLEncoder.encode(portalToken, "UTF-8") + "&serverURL="
	...
	private static ProxyConfig getConfig() throws IOException {
		...
		if (config != null)
			...
			String proxyFilePath = ProxyConfig.class.getClassLoader().getResource("/").getPath();
			throw new FileNotFoundException("The proxy configuration file should exists here:" + proxyFilePath);
		}
	}
	...
	public static class ProxyConfig {
		...
		public static boolean isUrlPrefixMatch(String prefix, String uri) {
			return uri.toLowerCase().startsWith(prefix.toLowerCase()) || uri.toLowerCase().replace("https://", "http://").startsWith(prefix.toLowerCase())
	...
	public static class ServerUrl {
		...
		public ServerUrl(String url, String matchAll, String oauth2Endpoint, String username, String password, String clientId, String clientSecret, String rateLimit, String rateLimitPeriod,
				String tokenServiceUri, String hostRedirect) {
	...
	private static void sendErrorResponse(HttpServletResponse response, String errorDetails, String errorMessage, int errorCode) throws IOException {
		...
		String message = "{" + "\"error\": {" + "\"code\": " + errorCode + "," + "\"details\": [" + "\"" + errorDetails + "\"" + "], \"message\": \"" + errorMessage + "\"}}";
	...
	try {
		try {
			...
			if (request.getHeader("referer") == null && allowedReferers != null && !allowedReferers[0].equals("*")) {
				...
				sendErrorResponse(response, "Current proxy configuration settings do not allow requests which do not include a referer header.", "403 - Forbidden: Access is denied.",
		...
		if (serverUrl.getRateLimit() > -1) {
			synchronized (_rateMapLock) {
				...
				if (!rate.click()) {
					...
					sendErrorResponse(response, "This is a metered resource, number of requests have exceeded the rate limit interval.", "Error 429 - Too Many Requests", 429);
